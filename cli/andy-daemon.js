  ...
  try {
    ...
    if (CEREBRAS_KEY && providerAvailable('cerebras')) {
      const text = await tryProvider('cerebras',
        () => callOpenAI('https://api.cerebras.ai', CEREBRAS_KEY, 'llama3.1-8b', prompt, maxTokens))
      if (text) return text
    }
...
async function executeTask(taskContent, taskName = '', isManual = false) {
  ...
  const fileOps = plan.split('\n')
    ...
    .filter(o => !(o.action === 'MODIFY' && [
      ...
      'src/context/AppContext.jsx', 'src/context/SettingsContext.jsx', 'src/context/AuthContext.jsx',
  ...
  for (const op of fileOps) {
    ...
    if (['jsx','tsx','js','ts'].includes(ext) && clean.length > 100) {
      ...
      if (/[,({+\-*/&|=<:>]$/.test(lastLine.trim()) || lastLine.trim().endsWith('...'))
        throw new Error(`JSX tronqué — termine par: "${lastLine.trim().slice(-40)}"`)
    }
